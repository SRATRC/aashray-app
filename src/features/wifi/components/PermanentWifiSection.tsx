...
import { PermanentWifiCode } from '../types';

import CustomAlert from '@/components/CustomAlert';
import CustomButton from '@/components/CustomButton';
import CustomErrorMessage from '@/components/CustomErrorMessage';
import CustomSelectBottomSheet, {
  CustomSelectBottomSheetRef,
} from '@/components/CustomSelectBottomSheet';
...
const PermanentWifiSection: React.FC<PermanentWifiSectionProps> = ({
  ...
  const getExpectedApprovalDate = (dateString?: string | null) => {
  ...
  const renderApprovedCode = (item: PermanentWifiCode) => (
  ...
  const renderPendingStatus = (item: PermanentWifiCode) => (
    <View className="gap-y-3">
  ...
  const renderRejectedStatus = (item: PermanentWifiCode) => (
    <View className="gap-y-3">
      <Text className="font-pmedium text-black">
  ...
  const renderResetStatus = (item: PermanentWifiCode) => (
    <View className="gap-y-3">
      <Text className="font-pmedium text-black">
  ...
  const renderWifiItem = (item: PermanentWifiCode, index: number) => (
    <View
      key={item.id || index}
