...
import { resolveTicket, sendTicketMessage, useTicketDetail } from '../api';
import { runUpload } from '../attachments';
...
import TicketResolvedBanner from '../components/TicketResolvedBanner';
import TicketStatusHeader from '../components/TicketStatusHeader';
...
const TicketDetailScreen = () => {
  ...
  const { data: ticket, isLoading, isError, refetch } = useTicketDetail(id, user.cardno);
  ...
  const existingVideoCount = useMemo(() => {
    if (!ticket) return 0;
    const perMessage = (ticket.messages || []).flatMap((m) => m.attachments || []);
    return [...(ticket.attachments || []), ...perMessage].filter(
      (a) => a?.kind === 'video' && !a.expired
    ).length;
  }, [ticket]);
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (payload: {
      ...
      return sendTicketMessage(id, user.cardno, {
  ...
  const resolveTicketMutation = useMutation({
    mutationFn: async () => {
      return resolveTicket(id, user.cardno);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ticket', id, user.cardno] });
  ...
  return (
    <SafeAreaView className="h-full w-full bg-white">
      <TicketStatusHeader
        ...
        showCloseButton={isTicketActive}
        onResolve={handleResolve}
        resolving={resolveTicketMutation.isPending}
      />
      ...
      <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
        ...
        {/* Fixed above the input (not part of the scrollable thread) so it's
            ...
            scrolling required. */}
        {isResolved && <TicketResolvedBanner />}
        ...
        <TicketComposer
          ...
          canAttach={canAddMedia && !isUploading}
          attachmentPreview={{
            attachments,
            onRemove: remove,
