...
const EventBookingDetails: React.FC<{ containerStyles: any }> = ({ containerStyles }) => {
  ...
  const formattedStartDate = moment(data.utsav.start_date).format('Do MMMM');
  const formattedEndDate = moment(data.utsav.end_date).format('Do MMMM, YYYY');
    ...
    <PrimaryAddonBookingCard title={'Raj Adhyayan Booking'} containerStyles={containerStyles}>
      <View className="item-center flex flex-row gap-x-4 p-4">
        ...
        <View className="w-full flex-1 justify-center gap-y-1">
          {data.validationData?.utsavDetails[0] && (
            <CustomTag
              text={data.validationData?.utsavDetails.status}
              textStyles={
                data.validationData?.utsavDetails.status == status.STATUS_AVAILABLE
                  ? 'text-green-200'
                  : 'text-red-200'
              }
              containerStyles={
